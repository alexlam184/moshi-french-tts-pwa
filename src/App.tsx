...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { speech.current.stop(); piperSpeech.current.stop(); supertonicSpeech.current.stop(); window.speechSynthesis?.removeEventListener('voiceschanged', refresh) }
  }, [])
  ...
  function applyText() {
    playlist.current = false
    stopEngines(); setPlayback(EMPTY_PLAYBACK)
  ...
  function changeEngine(next: EngineId) {
    playlist.current = false
    stopEngines(); setPlayback(EMPTY_PLAYBACK); setEngine(next)
  ...
  function changeVoice(next: string) {
    playlist.current = false
    stopEngines(); setPlayback(EMPTY_PLAYBACK); setVoice(next)
  ...
  function playSentence(index: number, fromWord = 0, continuing = false) {
    ...
    void speakWithSelectedEngine(selectedWords, {
      ...
      onError: error => { playlist.current = false; setPlayback(EMPTY_PLAYBACK); setStatus({ kind: 'stopped', title: 'Playback error', detail: error instanceof Error ? error.message : `${engine} could not generate audio.`, error: true }) },
    })
  ...
  function stop() { playlist.current = false; stopEngines(); setPlayback(EMPTY_PLAYBACK); setStatus({ kind: 'stopped', title: 'Stopped', detail: 'Playback was stopped.' }) }
  ...
  function playWord(sentenceIndex: number, wordIndex: number) {
    ...
    if (engine !== 'system' && !installed.includes(engine)) { setModelsOpen(true); setStatus({ kind: 'stopped', title: 'Voice unavailable', detail: `Install ${engine} first.`, error: true }); return }
    ...
    void speakWithSelectedEngine([word], {
      ...
      onError: error => setStatus({ kind: 'stopped', title: 'Playback error', detail: error instanceof Error ? error.message : `${engine} could not generate audio.`, error: true }),
    })
  }
  ...
  function hoverWord(sentenceIndex: number, wordIndex: number) {
    ...
    const repeat = () => {
      ...
      void speakWithSelectedEngine([repeatedWord], { onEnd: repeat, onError: error => setStatus({ kind: 'stopped', title: 'Playback error', detail: error instanceof Error ? error.message : `${engine} could not repeat the word.`, error: true }) })
    }
  ...
  return <div className="app-shell">
    ...
    {page === 'guide' ? <GuidePage onBack={() => setPage('practice')} /> : <main id="workspace" className="workspace">
      <section className="composer" aria-labelledby="composer-heading">
        ...
        <textarea id="french-text" value={draft} onChange={e => setDraft(e.target.value)} spellCheck lang="fr" aria-describedby="text-help" />
      ...
      <section className="listener" aria-labelledby="listener-heading">
        ...
        <div className="voice-controls">
          <label>engine<select value={engine} onChange={e => changeEngine(e.target.value as EngineId)}><option value="system">system / browser</option><option value="piper" disabled={!installed.includes('piper')}>piper{installed.includes('piper') ? '' : ' · install first'}</option><option value="supertonic" disabled={!installed.includes('supertonic')}>supertonic hd{installed.includes('supertonic') ? '' : ' · install first'}</option></select></label>
          ...
          <label>speed<select value={rate} onChange={e => setRate(Number(e.target.value))}>{[0.6, 0.75, 0.85, 1, 1.15].map(x => <option key={x} value={x}>{x}×</option>)}</select></label>
        </div>
        ...
        <div className="sentences">
          {sentences.length ? sentences.map((sentence, index) => <SentenceRow key={sentence.id} sentence={sentence} expanded={expanded.has(sentence.id)} active={playback.sentence === index} paused={playback.paused} activeWord={playback.sentence === index ? playback.word : -1} hoveredWord={playback.sentence === index ? playback.hovering : null} onToggle={() => setExpanded(current => { const next = new Set(current); next.has(sentence.id) ? next.delete(sentence.id) : next.add(sentence.id); return next })} onPlay={() => toggleSentence(index)} onWordClick={i => delayedWord(index, i)} onWordDoubleClick={i => startFromWord(index, i)} onWordEnter={i => hoverWord(index, i)} onWordLeave={leaveWord} />) : <div className="empty-state"><Icon name="speaker"/><p>add some french text to begin.</p></div>}
        </div>
      </section>
    </main>}
