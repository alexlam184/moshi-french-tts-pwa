...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    ...
    const firstResponse = await cachedFetch(voiceStylePaths[0]);
    ...
    // Fill in the data
    for (let i = 0; i < bsz; i++) {
        const response = await cachedFetch(voiceStylePaths[i]);
...
export async function loadCfgs(onnxDir) {
    const response = await cachedFetch(`${onnxDir}/tts.json`);
...
export async function loadTextProcessor(onnxDir) {
    const response = await cachedFetch(`${onnxDir}/unicode_indexer.json`);
